...
import { phatHienChotLoiTungPhan, tinhDongPhanConLai } from "@/lib/chotLoiTungPhan";
...
export function phatHienLenhDong({ dsMoi, banGhiCuTheoMa, ngayBan, daGhi = new Set() }) {
  ...
  for (const m of dsMoi) {
    ...
    if (cu.tp_da_cham === "TP3") {
      ...
      // Lenh theo cach moi khong bao gio vao nhanh nay (cham TP3 la dong lenh luon, xem nhanh CHOT_TP3 ben duoi).
      ketQua.push({
        ma: m.ma,
    ...
    if (lyDo === "CHOT_TP3") {
      ...
      for (const { cu: c3, m: m3 } of cascade.tp3KieuCu) {
        ...
        const r = tinhChotTP3({ giaMua: giaMua, tp1: tp[0], tp2: tp[1], tp3: tp[2], ngayMua: cu.ngay_mua_txt, ngayBan, soPhien: cu.so_phien_giu != null ? Number(cu.so_phien_giu) : null, tyLe: TY_LE_CHOT });
...
// tyLe: mac dinh CACH CU (30/30/25 = 85% vi the, con 15% chay); truyen TY_LE_CHOT (30/30/40 = 100%) cho lenh KET THUC o TP3 theo cach moi.
...
export async function layTPDaGhi(dsMa) {
  ...
  return withDb(async (client) => {
    ...
    const { rows } = await client.query(
      `SELECT ma, to_char(ngay_mua, 'YYYY-MM-DD') AS ngay_mua, vong FROM lenh_da_dong WHERE vong IN (5, 6) AND ma = ANY($1::text[])`,
      [dsMa]
    );
...
export function thongKeLenhDaDong(ds) {
  ...
  return {
    ...
    // Dong chot loi TUNG PHAN (TP1/TP2/TP3 kieu moi, phan_chot_pct < 100): luon la lai nen keo ty le thang len - dem rieng de nguoi xem biet.
    soChotMotPhan: ds.filter((x) => /^TP[123]$/.test(x.ly_do ?? "") && x.phan_chot_pct != null && Number(x.phan_chot_pct) < 100).length,
  };
}
