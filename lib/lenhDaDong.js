...
export async function doiSoatLenhDaDong({ apDung = true, ngayHomNay = null } = {}) {
  return withDb(async (client) => {
    ...
    const dkMoLai = `FROM lenh_da_dong d JOIN tin_hieu t ON t.ma = d.ma
         AND ((d.vong = 1 AND t.ngay_mua = d.ngay_mua) OR (d.vong = 2 AND t.dang_giu_moi IS TRUE AND t.ngay_mua_moi = d.ngay_mua))
       WHERE t.tin IN ('MUA', 'NAM GIU') AND d.ly_do IN ('BAN', 'THOAT', 'CAT_LO')`;
    const moLai = apDung
    ...
    if (apDung && ngayHomNay) {
      const r = await client.query(
        `UPDATE lenh_da_dong d SET gia_ban = t.gia, lai_lo_pct = (t.gia / d.gia_mua - 1) * 100
         FROM tin_hieu t
         WHERE t.ma = d.ma AND d.ly_do IN ('BAN', 'THOAT') AND d.ngay_ban = $1::date AND t.tin IN ('BAN', 'TRUNG LAP')
           AND t.gia > 0 AND d.gia_mua > 0 AND d.gia_ban IS DISTINCT FROM t.gia
         RETURNING d.ma`,
        [ngayHomNay]
      );
      capNhat = r.rows.map((x) => x.ma);
    }
