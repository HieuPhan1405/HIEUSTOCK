...
export async function dangKy({ sdt, matKhau, ten }) {
  ...
  return withDb(async (client) => {
    ...
    const { rows } = await client.query(
      `INSERT INTO nguoi_dung (sdt, mat_khau_hash, ten) VALUES ($1, $2, $3) RETURNING id, sdt, ten, la_admin`,
...
export async function dangNhap({ sdt, matKhau }) {
  ...
  return withDb(async (client) => {
    await daoDamBangNguoiDung(client);
    const { rows } = await client.query(`SELECT id, sdt, ten, la_admin, mat_khau_hash FROM nguoi_dung WHERE sdt = $1`, [
      sdtChuan,
    ]);
    const row = rows[0];
    ...
    return { token, hetHan, nguoiDung: { id: row.id, sdt: row.sdt, ten: row.ten, la_admin: row.la_admin } };
  });
}
...
export async function layNguoiDungTuToken(token) {
  ...
  return withDb(async (client) => {
    await daoDamBangNguoiDung(client);
    const { rows } = await client.query(
      `SELECT nd.id, nd.sdt, nd.ten, nd.la_admin FROM phien_dang_nhap pd
...
export async function capNhatTen(nguoiDungId, tenMoi) {
  ...
  return withDb(async (client) => {
    await daoDamBangNguoiDung(client);
    const { rows } = await client.query(`UPDATE nguoi_dung SET ten = $1 WHERE id = $2 RETURNING id, sdt, ten, la_admin`, [sach, nguoiDungId]);
    return rows[0] || null;
  });
}
...
export async function layTatCaNguoiDung() {
  return withDb(async (client) => {
    await daoDamBangNguoiDung(client);
    const { rows } = await client.query(
      `SELECT id, sdt, ten, la_admin, tao_luc FROM nguoi_dung ORDER BY tao_luc DESC LIMIT 500`
    );
    return rows;
  });
}
