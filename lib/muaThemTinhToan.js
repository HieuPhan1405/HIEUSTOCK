...
export function phanConLaiLenhGoc(tpDaCham) {
  // Cach 2 TP + giu den BAN: TP3 chi la moc tham khao nen phan con lai cua lenh goc van la 40% (nhu sau TP2).
  if (tpDaCham === "TP3") return TY_LE_CHOT.giu;
...
export const LOAI_DIEM_MUA = {
  moi: { nhan: "Lệnh mới sau khi lệnh gốc chạm TP3", ngan: "Mua mới sau TP3" },
  giua: { nhan: "Giữa chừng (trước TP3)", ngan: "Mua thêm giữa chừng" },
};

// Nhan ngan hien tren dong mua them trong bang lenh (chi lenh mua them GIUA CHUNG): "Mua thêm giữa chừng".
...
// 1 DIEM MUA MOI thanh 1 "dong lenh" cua So lenh dang mo. MUA THEM GIUA CHUNG (vong "giua", la_mua_them = true) la lenh phu gan voi lenh goc; MUA MOI SAU TP3 (vong "moi", sau_tp3 = true)
// la LENH MOI BINH THUONG (gia mua, Stop-loss, TP rieng - chạm TP3 la ket thuc lenh cu, lenh moi mo len sau do) nen la 1 dong thuong, khong phai mua them. dung lai dung cac cot cua bang (gia mua, vung mua/cat lo/chot loi, lai/lo...) nen dong nay la ban sao
...
export function gopLenhMo(dangMo) {
  return dangMo.flatMap((r) => {
    ...
    return [{ ...r, khoa_lenh: r.ma, la_mua_them: false, co_mua_them: diem.filter((d) => d.vong === "giua").length }, ...diem.map((d) => dongTuDiemMua(r, d))];
  });
}

// DANH SACH LENH DANG MO (So lenh dang mo / Danh muc theo doi) tu cac dong tin hieu da chuan hoa: lenh goc cua ma dang MUA/NAM GIU + cac lenh phu (mua them giua chung, lenh moi sau TP3)
// dang giu. Ma da ve TRUNG LAP (vd cham TP3 = ket thuc lenh) khong con lenh goc nhung van giu lai lenh phu cua no neu con dang giu.
export function lenhDangMo(ds) {
  const giu = (r) => r.tin === "MUA" || r.tin === "NAM GIU";
  const coLenhPhu = (r) => r.dang_giu_moi === true || r.dang_giu_giua === true || r.mua_moi === true || r.mua_giua === true;
  return gopLenhMo(ds.filter((r) => giu(r) || coLenhPhu(r))).filter((r) => r.la_mua_them || r.sau_tp3 || giu(r));
}
