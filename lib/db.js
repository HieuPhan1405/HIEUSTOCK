...
// Bang tin hieu: "tin_hieu" la du lieu THAT hien tren web; "tin_hieu_chikou" chua ket qua Explore cua
// file AFL 10 (ban thu Chikou thong thoang) de xem so sanh o /thu-nghiem-chikou ma KHONG dung toi du
// lieu that. Ten bang chi nhan tu danh sach nay (khong bao gio noi truc tiep chuoi tu ben ngoai vao SQL).
export const BANG_TIN_HIEU = { thuong: "tin_hieu", chikou: "tin_hieu_chikou" };

export function tenBangTinHieu(khoa) {
  return BANG_TIN_HIEU[khoa] || BANG_TIN_HIEU.thuong;
}

export async function daoDamBangTinHieu(client, ten = "tin_hieu") {
  if (!Object.values(BANG_TIN_HIEU).includes(ten)) throw new Error("Ten bang tin hieu khong hop le");
  await client.query(`
