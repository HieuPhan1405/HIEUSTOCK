...
async function tuNenHomNayDangChay(ma) {
  ...
  if (!Array.isArray(j.t) || j.t.length === 0) return null;
  const homNay = ngayIso(den);
  const nenPhut = j.t.map((t, i) => ({ t, o: j.o[i], h: j.h[i], l: j.l[i], c: j.c[i], v: j.v?.[i] ?? 0 })).filter((b) => nenHopLe(b) && ngayIso(b.t) === homNay);
