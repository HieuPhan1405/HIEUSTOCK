...
// khoaBang: "thuong" (mac dinh, du lieu that) hoac "chikou" (ban thu Chikou thong thoang).
export async function layTatCaTinHieu(khoaBang = "thuong") {
  const bang = tenBangTinHieu(khoaBang);
  return withDb(async (client) => {
    await daoDamBangTinHieu(client, bang);
    const { rows } = await client.query(
      `SELECT ${CAC_COT} FROM ${bang} WHERE ma != 'VNINDEX' ORDER BY diem DESC NULLS LAST`
    );
