...
function chuanHoaHang(row) {
  ...
  // CHAM TP3 = KET THUC LENH: khong con nam vi the nen hien TRUNG LAP (khong phai BAN / NAM GIU). Hai truong hop: (1) phien vua khop TP3 (AFL/engine bao BAN voi ly_do_ban = 5)
  // -> ket_thuc_tp3_moi = true (dung cho cot "Ban bot" o trang dau, chi hien trong ngay); (2) lenh CU con giu phan chay sau TP3 (tin MUA/NAM GIU, tp_da_cham = TP3) - AFL cu chua
  // ket thuc lenh o TP3, cho toi khi Explore lai bang AFL moi. Ca 2 deu gan ket_thuc_tp3 = true de web ghi ro ly do "Cham TP3 - ket thuc lenh".
  const vuaKhopTP3 = row.tin === "BAN" && Number(row.ly_do_ban) === 5;
  if (vuaKhopTP3 || (dangGiu && row.tp_da_cham === "TP3")) {
    ketQua.tin = "TRUNG LAP";
    ketQua.ket_thuc_tp3 = true;
    ketQua.ket_thuc_tp3_moi = vuaKhopTP3;
  }
  return ketQua;
}
