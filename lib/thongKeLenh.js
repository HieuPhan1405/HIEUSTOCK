...
const trongSo = (x) => (Number(x.phan_chot_pct) > 0 ? Number(x.phan_chot_pct) : 100) / 100;
...
const loaiLenh = (x) => (x.vong === 2 ? "moi" : x.vong === 4 ? "giua" : "goc");
...
// Dong nay co KET THUC lenh khong? Dong dong that su (BAN / cat lo / thoat / thoat Kijun / chot du TP3 du phong) luon ket thuc; dong TP chi ket thuc khi la TP3 cach moi
// (40% vi the, hoac gop 100%) - TP1, TP2 va TP3 cach cu (25% / gop 85%, con 15% giu chay) thi lenh van con phan giu.
function ketThucLenh(x) {
  if (!laDongTP(x.ly_do)) return true;
  if (x.ly_do !== "TP3") return false;
  const w = trongSo(x);
  return Math.abs(w - TY_LE_CHOT.tp3 / 100) < 1e-9 || w >= 0.999;
}
...
// Thong ke tong hop cho trang Lenh da dong: chi tinh cac lenh DA DONG HAN (moi lenh 1 lan). Cac lenh moi chot 1 phan (TP1/TP2, hoac TP3 cach cu con phan chay)
// dem rieng o soDangChotTungPhan.
