...
// Cac ma nguoi dung da tham gia KEM thoi diem tham gia - cho Danh muc theo doi
// (loc lenh da dong chi tinh tu luc tham gia).
export async function layThamGiaCuaToi(nguoiDungId) {
  if (!nguoiDungId) return [];
  return withDb(async (client) => {
    await daoDamBangThamGia(client);
    const { rows } = await client.query(
      `SELECT ma, to_char(tao_luc AT TIME ZONE 'Asia/Ho_Chi_Minh', 'YYYY-MM-DD') AS ngay_tham_gia
       FROM tham_gia_ma WHERE nguoi_dung_id = $1 ORDER BY tao_luc DESC`,
      [nguoiDungId]
    );
    return rows;
  });
