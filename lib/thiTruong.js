...
export function tinhTongQuanThiTruong(ds, vnindex) {
  ...
  // Moi diem MUA MOI (dot sau) = 1 dong { ma, vong: "giua" | "moi", giaMua, gia (hien tai), homNay,... } - la lenh rieng (xem lib/muaThemTinhToan.js).
  const diemMua = ds.flatMap(cacDiemMuaMoi);
  const lenh = {
    mua,
    muaMoiHomNay: diemMua.filter((d) => d.homNay),
    ban: banThuong,
    // Cham TP3 = ket thuc lenh, khong con nam vi the -> tin hieu da chuan hoa ve TRUNG LAP (lib/tinHieu.js); ket_thuc_tp3_moi = vua khop TP3 trong phien nay (chi tinh trong ngay,
    // khong gom lenh cu da cham TP3 tu truoc).
    ketThucTP3: ds.filter((r) => r.ket_thuc_tp3_moi),
    thoatKijun: ban.filter((r) => lyDoBan(r) === 4),
    banBot: ds.filter((r) => dangGiuLenh(r) && r.ban_bot),
    // Cach 2 TP + giu den BAN: TP3 chi la moc tham khao, lenh da cham TP3 van con giu (da chot TP1 30% + TP2 30%).
