...
export function tinhTongQuanThiTruong(ds, vnindex) {
  ...
  // LENH MUA-BAN cua he thong: mua, mua them (mua moi sau TP3), ban, ban bot, chot loi theo moc TP da cham (dang giu lenh).
  const dangGiuLenh = (r) => r.tin === "MUA" || r.tin === "NAM GIU";
  const lenh = {
    mua,
    muaThemHomNay: ds.filter((r) => r.mua_moi === true),
    dangMuaThem: ds.filter((r) => r.dang_giu_moi === true && r.mua_moi !== true),
    ban,
    ...
    coDuLieuMuaThem: ds.some((r) => r.dang_giu_moi != null),
  };
...
// Tong GTGD + KL 1 ngay TOAN SAN HOSE = tong nmValue/nmVolume TAT CA ma (VNDirect stock_prices, cung ho
// API v4 dang dung o layDongTienNuocNgoai() va lib/lichSuGia.js). nmValue/nmVolume cua ngay HOM NAY la
// so CHAY (cap nhat trong phien, da doi chieu truc tiep), khong phai chi co sau khi dong cua.
async function tongGiaoDichHOSENgay(ngayISO, laHomNay) {
  try {
    const res = await fetch(
      `https://api-finfo.vndirect.com.vn/v4/stock_prices?q=date:${ngayISO}~floor:HOSE&size=500&fields=code,nmValue,nmVolume`,
      { next: { revalidate: laHomNay ? 20 : 86400 }, signal: AbortSignal.timeout(12000) }
    );
    if (!res.ok) return null;
    const j = await res.json();
    const data = (j.data || []).filter((x) => Number.isFinite(x.nmValue));
    if (data.length < 200) return null; // chua co du lieu ngay do (ngay le, cuoi tuan lo query, hoac qua som trong phien)
    return {
      ngay: ngayISO,
      giaTriTy: data.reduce((s, x) => s + x.nmValue, 0) / 1e9,
      klTrieu: data.reduce((s, x) => s + (x.nmVolume || 0), 0) / 1e6,
    };
  } catch {
    return null;
  }
...
// Chuoi GTGD toan san HOSE (tang dan theo ngay) N phien gan nhat, bo T7/CN - dung cho bieu do Thanh
// khoan o dashboard (duong TB20, so sanh voi phien truoc). Phan tu CUOI la hom nay (so dang chay).
export async function layChuoiThanhKhoanHOSE(soPhien = 21) {
  const tz = "Asia/Ho_Chi_Minh";
  const homNayISO = new Date().toLocaleDateString("en-CA", { timeZone: tz });
  const ngayList = [];
  for (let lui = 0; ngayList.length < soPhien && lui < soPhien * 2 + 10; lui++) {
    const d = new Date(Date.now() - lui * 86400e3);
    const thu = d.toLocaleDateString("en-US", { weekday: "short", timeZone: tz });
    if (thu === "Sat" || thu === "Sun") continue;
    ngayList.push(d.toLocaleDateString("en-CA", { timeZone: tz }));
  }
  ngayList.reverse();
  const ketQua = await Promise.all(ngayList.map((n) => tongGiaoDichHOSENgay(n, n === homNayISO)));
